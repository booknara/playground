...
public class MergeIntervals {
   ...
   * 1  --  3
   *     2 ---- 6
   * Sort the array based on the start value of the intervals
   * 3 cases
   * 1) next interval start later than current end
   * -> finish current interval and next interval = new current interval
   * 2) next interval start before than current end
   * 2-1) current end > next end -> skip next interval
   * 2-2) current end < next end -> next end becomes new current end
   */
  // T:O(n*logn) S:O(n)
